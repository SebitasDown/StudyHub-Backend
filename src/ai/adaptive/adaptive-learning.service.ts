...
export class AdaptiveLearningService {
  ...
  analyze(userMessage: string, academicContext: any, memories: any[], analytics: any[], knowledgeGaps: any[]) {
    ...
    const mode = this.socratic.decideMode(userMessage);
    const predictedWeaknesses = this.weakness.predictFromGaps(knowledgeGaps || []);

    const actions: any = {};
    if (mode === 'COACH') {
      actions.resources = this.resources.generateQuickResources(userMessage, model.level);
      actions.exam = this.exam.generateMockExam(userMessage, 5);
    }
    if (mode === 'SOCRATIC') {
      actions.socraticPrompts = this.socratic.generateSocraticPrompts(userMessage);
    }
